...
async function launch(opts = {}) {
  ...
  const KEYCODES = { Enter: 13, Escape: 27, ArrowUp: 38, ArrowDown: 40,
                     ArrowLeft: 37, ArrowRight: 39, ' ': 32 };
  const keyParams = key => ({
    key, code: key === ' ' ? 'Space' : (key.length === 1 && /[a-z]/i.test(key) ? 'Key' + key.toUpperCase() : key),
  ...
  const page = {
    ...
    /* held keys: movement needs press and release to be separate events */
    async keyDown(key) {
      await cmd('Input.dispatchKeyEvent', { type: 'keyDown', ...keyParams(key) });
      await sleep(10);
    },
    async keyUp(key) {
      await cmd('Input.dispatchKeyEvent', { type: 'keyUp', ...keyParams(key) });
      await sleep(10);
    },
